...
//删除数据表中的数据
function del(gridId, url, opts) {
    var grid = mini.get(gridId);
    if (grid) {
        mini.confirm("确定删除数据么?", "删除数据", function (e) {
            if (e == "ok") {
...
function gridSave(gridId, url, opts, button) {
    ...
    if (grid && validateAndHint(grid)) {
        ...
        for (var i in changes) {
            ...
            changes[i].editState = changes[i]._state;
            delete changes[i]._state;
            delete changes[i]._id;
            delete changes[i]._level;
            delete changes[i]._pid;
            delete changes[i]._uid;
        }
...
function dealChanges(changes) {
    if ($.isArray(changes)) {
        for (var i = 0; i < changes.length; i++) {
            dealChanges(changes[i]);
        }
    } else {
        if (changes instanceof Date) {
            changes.format('yyyy-MM-dd HH:mm:ss');
        }
    }
}
                ...
                success: function (data) {
                    ...
                    if (code == 1)
                        ...
                        if (opts.errorShowLevel == 'parent' && window.parent)
                            window.parent.showMsg(message);
                        else if (opts.errorShowLevel == 'top' && window.top)
                            window.top.showMsg(message);
                        else
                            showMsg(message);
                    }
                },
        ...
        $.ajax({
            ...
            success: function (result) {
                ...
                if (result && code == 1) {
                    if (opts.isHint && message)
                        hint(message);
                    handleAction(opts.action, result);
                }
...
function showMsgError(msg, callback) {
    if (!msg)
        msg = "";
    var id = Math.ceil(Math.random() * 1000000);
    $('body').append("<div id='miniui_showMsg_window_" + id + "' class='mini-window' title='错误提示' style='' showMaxButton='true' showFooter='true'"
        + "showMaxButton='true' showCollapseButton='true' showShadow='true' allowResize='true' allowDrag='true' width='300px' style='z-index:10000'>"
        + "<div property='footer' style='text-align:center;padding:5px;padding-right:15px;'>"
        + "<input type='button' value='关闭' onclick='mini.get(\"miniui_showMsg_window_" + id + "\").destroy();' style='vertical-align:middle;'/>"
        + "</div>"
        ...
        + "<tbody>"
        + "<tr>"
        + "<td><div class='mini-messagebox-error' style=''></div></td>"
        + "<td class='mini-messagebox-content-text' style='width: 100%;'>"
        + '<div style="text-align:left;width:200px;white-space: normal;word-wrap: break-word; word-break: normal;">'
        + msg
        + '</div>'
...
function showValidateMsg(msg, callback) {
    if (!msg)
        msg = "";
    var id = Math.ceil(Math.random() * msg);
    var result = "";
    $.each(JSON.parse(msg), function (idx, item) {

        result += "第" + item.index + "项:";
    ...
    $('body').append("<div id='miniui_showMsg_window_" + id + "' class='mini-window' title='验证提示' style='' showMaxButton='true' showFooter='true'"
        + "showMaxButton='true' showCollapseButton='true' showShadow='true' allowResize='true' allowDrag='true' width='300px' style='z-index:10000'>"
        + "<div property='footer' style='text-align:center;padding:5px;padding-right:15px;'>"
        + "<input type='button' value='关闭' onclick='mini.get(\"miniui_showMsg_window_" + id + "\").destroy();' style='vertical-align:middle;'/>"
        + "</div>"
